...
function renderTable(sheetName, keepPage) {
    ...
    // ZETTBOT FIX: Menghapus else { displayData.reverse(); } karena displayData sudah terurut (Descending ID) dari script-core.js
...
function updateDashboard() {
    ...
    // ZETTBOT FIX: Mengambil 5 data pertama langsung dari array yang sudah ter-sortir. Hapus .reverse().
    ...
    var successHandler = function(result) {
        ...
        try {
            if (result.success) {
                ...
                // ZETTBOT SAFEGUARD: Paksa Sorting setiap kali data diperbarui dari Form
                if (typeof sortDataByIdDesc === 'function') newData = sortDataByIdDesc(newData);
                ...
                if(result.pelanggan) { 
                    var cleanPlg = result.pelanggan.map(function(p) { if (p['No Telpon']) { var hpStr = String(p['No Telpon']); if (hpStr.startsWith("'")) { hpStr = hpStr.substring(1); } p['No Telpon'] = hpStr; } return p; }); 
        ...
        google.script.run.withSuccessHandler(function(res) {
            try {
                if(res.success) {
                    ...
                    // ZETTBOT SAFEGUARD: Sort Descending saat delete
                    if (typeof sortDataByIdDesc === 'function') newData = sortDataByIdDesc(newData);
                    
                    if (res.pelanggan) {
                         var cleanPlg = res.pelanggan.map(function(p) { if (p['No Telpon']) { var hpStr = String(p['No Telpon']); if (hpStr.startsWith("'")) { hpStr = hpStr.substring(1); } p['No Telpon'] = hpStr; } return p; });
                         appData.pelanggan = typeof sortDataByIdDesc === 'function' ? sortDataByIdDesc(cleanPlg) : cleanPlg;
                    }
...
function renderHistoryPelanggan() {
    ...
    if (filteredTx.length === 0) {
        ...
        // ZETTBOT FIX: Menghapus .reverse() karena appData.produksi sudah diurutkan (Descending) secara global
