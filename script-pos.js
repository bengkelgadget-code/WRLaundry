            ...
            get run() {
                return {
                    ...
                    saveTransaksiStaff: function(p1, p2) {
                        // Delegasi logika diserahkan sepenuhnya ke script-pos.js
                        // Fungsi ini murni untuk jembatan backend jika dipanggil dari luar
                        ...
                        .then(res => res.json()).then(resData => {
                            if (this._onSuccess) this._onSuccess(resData);
                        }).catch(e => {
                            if (this._onFailure) this._onFailure(e);
                        });
                        return this;
                    },
...
// ZETTBOT FIX: Hindari Error "InvalidStateError" di Input File/Angka/Password!
// GLOBAL INPUT UPPERCASE (Bypass Login/Users)
document.addEventListener('input', function(e) {
    if (e.target && (e.target.tagName === 'INPUT' || e.target.tagName === 'TEXTAREA')) {
        // ZETTBOT FIX: Abaikan tipe input yang bisa menyebabkan exception (file, password, email, number)
        ...
        if (oldVal !== newVal) {
            ...
            // Cegah error cursor-jump dengan try-catch
...
// PULL TO REFRESH
var startY = 0;
document.addEventListener('touchstart', function(e) { if(window.scrollY < 10) startY = e.touches[0].pageY; });
