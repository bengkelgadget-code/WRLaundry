...
// ZETTBOT FIX: Native Scroll Into View dengan Spacer Dinamis (Anti Tabrakan Keyboard PWA)
    ...
    if (el && !el.dataset.scrollAttached) {
        ...
        el.addEventListener('focus', function() {
            ...
            if (scrollArea && footer) {
                // Suntikkan ruang kosong ekstra (landasan absolut) di bawah form
                scrollArea.style.paddingBottom = '75vh'; 
                var executeScroll = function() {
                    // Perintahkan browser menaruh tepi bawah footer sejajar dengan tepi bawah layar yang terlihat
                    footer.scrollIntoView({ behavior: 'smooth', block: 'end' });
                };
                // Multi-trigger menyesuaikan durasi pop-up keyboard Android/iOS yang berbeda-beda
        ...
        el.addEventListener('blur', function() {
            setTimeout(function() {
                ...
                if (!act || (act.id !== 'staff-input-diskon' && act.id !== 'staff-input-pembayaran' && act.id !== 'staff-input-dp')) {
                    var scrollArea = document.getElementById('staff-modal-scroll-area');
                    // Cabut landasan agar scroll tidak terlalu panjang ke bawah saat tidak dibutuhkan
